...
function LessonNode({ lesson, index }: { lesson: Lesson; index: number }) {
  ...
  const content = (
    ...
    >
      <div style={{
        flex: 1,
        background: 'var(--card)',
        border: `1px solid ${lesson.status === 'available' ? 'rgba(255,107,53,0.4)' : 'var(--border)'}`,
        borderRadius: 14,
        padding: '14px 18px',
        opacity: lesson.status === 'locked' ? 0.5 : 1,
      }}>
        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 4 }}>
          {lesson.type === 'quiz' ? '📝 Quiz' : lesson.type === 'challenge' ? '⚔️ Desafío' : '📖 Lección'}
        </div>
  ...
  const nodeButton = lesson.status !== 'locked' ? (
    <Link to={lesson.type === 'quiz' ? `/quiz/${lesson.id}` : `/learn/${lesson.id}`} style={{ textDecoration: 'none' }}>
      <motion.div
        ...
        style={{
          ...
          boxShadow: lesson.status === 'available' ? '0 0 0 6px rgba(255,107,53,0.2), 0 0 0 12px rgba(255,107,53,0.1)' : 'none',
          flexShrink: 0,
          cursor: 'pointer',
...
function ModuleSection({ module }: { module: Module }) {
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {/* Vertical path connector + lessons */}
...
export function Dashboard() {
  ...
  const { modules, loadModules, dailyStreak, completedLessons } = useProgressStore();
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Nivel {user.level} · {user.city}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  {dailyStreak > 0 ? (
                    <span className="streak-fire" style={{ fontSize: '1.4rem' }}>🔥</span>
                  ) : (
                    <span>🐕</span>
                  )}
                  <span style={{ fontWeight: 900, fontSize: '1.3rem', color: '#FF6B35' }}>{dailyStreak}</span>
                </div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>días seguidos</div>
              </div>
              ...
              <div style={{ minWidth: 140 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Zap size={12} color="#FFD700" /> {user.xp} XP
                  </span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Niv. {user.level + 1}: {xpToNextLevel}</span>
                </div>
              ...
              {/* XLM */}
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.3rem', fontWeight: 900, color: 'var(--accent)' }}>⭐ {user.xlmEarned}</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>XLM ganados</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <span className="streak-fire" style={{ fontSize: '2.5rem' }}>🔥</span>
                <div>
                  <div style={{ fontWeight: 900, fontSize: '1.1rem' }}>Desafío Diario</div>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Completa 1 lección hoy para mantener tu racha</div>
                </div>
              </div>
              <Link to="/learn/lesson-3" className="btn btn-primary">
                ¡Acepto el reto! <ChevronRight size={16} />
              </Link>
            </motion.div>

            {/* Module path */}
            <h2 style={{ fontSize: '1.4rem', fontWeight: 900, marginBottom: 8, textAlign: 'center' }}>Tu camino de aprendizaje 🗺️</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {dailyStreak > 0 && (
              <motion.div
                ...
                className="card"
                style={{ textAlign: 'center', padding: 24, border: '1px solid rgba(255,107,53,0.3)' }}
              >
                <span className="float-animation" style={{ fontSize: '3.5rem', display: 'block', marginBottom: 12 }}>🐕</span>
                <div style={{ fontWeight: 900, fontSize: '1.1rem', marginBottom: 4 }}>¡{dailyStreak} días seguidos!</div>
                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 12 }}>
                  Xollo está muy orgulloso de ti
                </div>
                <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {mockLeaderboard.slice(0, 5).map((entry) => (
                  <div key={entry.userId} style={{
                    ...
                    borderRadius: 10,
                    border: entry.isCurrentUser ? '1px solid rgba(255,107,53,0.3)' : '1px solid transparent',
                  }}>
                    <div style={{
